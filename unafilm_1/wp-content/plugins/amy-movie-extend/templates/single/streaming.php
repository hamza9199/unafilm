...
$prefix     = $movie->get_stream_prefix_name();
...
$query_movie    = get_page_by_path($slug, OBJECT, 'amy_movie');
$servers        = get_post_meta($query_movie->ID, 'servers', true);
?>
        ...
        <div class="amy-movie-item-play">
            <div class="container">
                <div class="box-player" id="amyStream">
                    ...
                    ?>
                    <?php if (strpos($video_link, 'youtube') == true || strpos($video_link, 'vimeo') == true) : ?>
                    ...
                    <?php else : ?>
                        <div class="amy-iframe">
                            <iframe src="<?php echo ($video_link); ?>" allowfullscreen></iframe>
                        </div>
                    <?php endif; ?>
                            ...
                            <div class="amy-movie-serve-item">
                                <div class="amy-movie-serve-name">
                                    <span>
                                        ...
                                        <?php echo esc_attr($server['title']); ?>
                                    </span>
                                </div>
                                <?php if (!empty($server['parts'])) : ?>
                                    <?php $i = 0; ?>
                                    <div class="amy-movie-serve-options">
                                        <?php foreach ($server['parts'] as $part) : ?>
                                            <?php
                                                if ($i == 0 && $j == 0) {
                                                    $classactive = 'active';
                                                } else {
                                                ...
                                                if (strpos($part['link'], 'youtube') == true) {
                                                    $data_type = 'youtube';
                                                } else if (strpos($part['link'], 'vimeo.com') == true) {
                                                    $data_type = 'vimeo';
                                                } else {
                                                    $data_type = 'video';
                                                }
                                            ?>
                                            <span class="optinal-link amy-streaming-link <?php echo esc_attr($classactive); ?>" data-type="<?php echo esc_attr($data_type); ?>" data-source="<?php echo esc_url($part['link']); ?>">
                                                <?php echo esc_attr($part['title']); ?>
                                            </span>
